...
function parseArguments(argument) {
    ...
    if (params.url) {
        const rawUrl = sanitizeTemplateValue(params.url);
        const rawName = sanitizeTemplateValue(params.title || params.name || "");
        const rawResetDay = sanitizeTemplateValue(params.reset_day || params.resetDay || "");
        slots.push(extractUrlAndOptions(rawUrl, rawName, rawResetDay));
    }

    for (let index = 1; index <= MAX_SUBSCRIPTIONS; index += 1) {
        const rawUrl = sanitizeTemplateValue(
            params[`url${index}`] || params[`URL${index}`] || params[`订阅链接${index}`] || ""
        );
        const rawName = sanitizeTemplateValue(
            params[`title${index}`] ||
            params[`name${index}`] ||
            params[`NAME${index}`] ||
            params[`机场名称${index}`] ||
        ...
        const rawResetDay = sanitizeTemplateValue(
...
function parseSlotPayload(payload) {
    ...
    return payload
        ...
        .map((entry) => {
            ...
            return extractUrlAndOptions(
                sanitizeTemplateValue(url),
                sanitizeTemplateValue(name),
                sanitizeTemplateValue(resetDay)
            );
        })
...
function extractUrlAndOptions(urlStr, originalName, originalResetDay) {
    let finalUrl = String(urlStr || "").trim();
